...
The spec asks for Framer Motion — which is React-only and can't run in this
...
**This changed the folder structure — replace your whole `gmt-hotel`
folder with this download rather than copying individual files, or
you'll end up with two copies of the assets folder.**

## Guest/Room/reservation search fixed — a bigger sweep than last time

The earlier PDOException sweep only checked SQL written as one literal
string directly inside a `->prepare()` call. It **missed a whole other
version of the same bug**: search filters built by appending pieces onto
a `$where` variable first — e.g. `'(g.full_name LIKE :s OR g.phone LIKE
:s OR g.email LIKE :s)'` — where the same `:s` placeholder covers three
different `OR` conditions but only one value ever gets bound to it. That
pattern is used in almost every search box in the app, which is exactly
why guest search, room search, and the reservation guest-picker all broke
together — they all share this one bad idiom.

Found and fixed **13 occurrences** this time, across Guests, Reservations,
Check-In, Check-Out, Events, Payments, Invoices, Restaurant, Rooms,
Staff, and Audit Logs — every place a search box filters across multiple
columns. Each now uses a distinct placeholder per condition (`:s1`, `:s2`,
`:s3`...) bound to the same value, which is what PDO's strict prepared
statements actually require.

I've kept the detection script; going forward I'll run it after any batch
of query changes rather than trusting a single earlier clean pass.
...
- **"Send via WhatsApp"** — on Check-In, Check-Out, and the Invoices list.
  **Important limitation, not a bug**: WhatsApp's `wa.me` links can only
  pre-fill a text message — there's no way for a webpage to make WhatsApp
  auto-attach a file. True one-click auto-send requires the official
  WhatsApp Business Cloud API, which needs a Meta-verified business phone
  number and API credentials — an external account you'd have to set up
  yourself, not something buildable from here. What's actually built: one
  click downloads the real PDF **and** opens WhatsApp with the guest's
  number and a message already typed — front desk attaches the file in
  that same chat. Phone numbers are auto-converted from local format
  (e.g. `0805...`) to international using the country code set in
  **Settings → WhatsApp** (defaults to `234` for Nigeria — change it there
  if needed, it's not hardcoded).
...
- **Test every module against a real MySQL database.** I have no PHP
  runtime in my environment, so none of this has actually been executed —
  only carefully reviewed. Click through every CRUD flow, every export,
  every role, before trusting it with real bookings.
