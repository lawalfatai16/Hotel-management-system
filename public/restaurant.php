  ...
  <div class="flex-1 min-w-0">
    ...
    <main class="p-4 md:p-8 space-y-6">
      ...
      <div class="flex rounded-lg border border-[--brand-ink]/10 overflow-hidden w-fit">
        <button id="tab-pos" onclick="switchTab('pos')" class="px-4 py-2 text-sm font-medium bg-[--brand-coffee] text-[--brand-cream]">New Order</button>
        <button id="tab-orders" onclick="switchTab('orders')" class="px-4 py-2 text-sm font-medium">Orders</button>
        <button id="tab-menu" onclick="switchTab('menu')" class="px-4 py-2 text-sm font-medium">Menu</button>
        <button id="tab-tables" onclick="switchTab('tables')" class="px-4 py-2 text-sm font-medium">Tables</button>
      </div>
      ...
      <div id="panel-pos" class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 card-surface p-6">
          ...
          <input id="menuSearch" type="text" placeholder="Search menu…" class="w-full mb-4 rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
        ...
        <div class="card-surface p-6 h-fit sticky top-24">
          ...
          <div class="flex rounded-lg border border-[--brand-ink]/10 overflow-hidden mb-4 text-xs">
            <button type="button" onclick="setOrderType('dine_in')" id="type-dine_in" class="flex-1 py-2 bg-[--brand-coffee] text-[--brand-cream]">Dine-in</button>
            <button type="button" onclick="setOrderType('room_charge')" id="type-room_charge" class="flex-1 py-2">Room Charge</button>
            <button type="button" onclick="setOrderType('takeaway')" id="type-takeaway" class="flex-1 py-2">Takeaway</button>
          </div>
          <div id="tablePickerWrap" class="mb-4">
            <label class="block text-xs font-medium text-[--brand-muted] mb-1.5">Table</label>
            <select id="tableSelect" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm"></select>
          </div>
          <div id="reservationPickerWrap" class="mb-4 hidden relative">
            ...
            <input id="reservationSearch" autocomplete="off" placeholder="Search reservation or room…" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
          ...
          <div class="border-t border-[--brand-ink]/10 pt-3 space-y-1 text-sm mb-4">
            <div class="flex justify-between"><span class="text-[--brand-muted]">Subtotal</span><span id="cartSubtotal">—</span></div>
            <div class="flex justify-between"><span class="text-[--brand-muted]">Tax</span><span id="cartTax">—</span></div>
            <div class="flex justify-between font-semibold"><span>Total</span><span id="cartTotal">—</span></div>
          </div>
      ...
      <div id="panel-orders" class="hidden space-y-4">
        <div class="card-surface p-4 flex flex-wrap gap-3 items-center">
          <input id="orderSearch" type="text" placeholder="Search order, guest, or reservation…" class="flex-1 min-w-[200px] text-sm rounded-lg border border-[--brand-ink]/10 px-3 py-2">
          <select id="orderStatusFilter" class="text-sm rounded-lg border border-[--brand-ink]/10 px-3 py-2">
...
<div id="categoryModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
  <div class="glass-panel rounded-2xl w-full max-w-sm p-6">
    ...
    <form id="categoryForm" class="space-y-4">
      <input name="name" required placeholder="e.g. Main Course" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
...
<div id="itemModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
  <div class="glass-panel rounded-2xl w-full max-w-md p-6">
    ...
    <form id="itemForm" class="space-y-4">
      ...
      <select id="item_category_id" name="category_id" required class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm"></select>
      <input name="name" id="item_name" required placeholder="Item name" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
      <input type="number" name="price" id="item_price" min="0" step="0.01" required placeholder="Price" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
      <textarea name="description" id="item_description" rows="2" placeholder="Description (optional)" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm"></textarea>
...
<div id="tableModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
  <div class="glass-panel rounded-2xl w-full max-w-sm p-6">
    ...
    <form id="tableForm" class="space-y-4">
      <input name="table_number" required placeholder="Table number, e.g. T-01" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
      <input type="number" name="capacity" min="1" value="4" placeholder="Capacity" class="w-full rounded-lg border border-[--brand-ink]/10 px-3 py-2 text-sm">
...
function switchTab(tab) {
  ['pos','orders','menu','tables'].forEach(t => {
    ...
    document.getElementById(`tab-${t}`).className = `px-4 py-2 text-sm font-medium ${t===tab ? 'bg-[--brand-coffee] text-[--brand-cream]' : ''}`;
  });
...
function setOrderType(type) {
  ...
  ['dine_in','room_charge','takeaway'].forEach(t => {
    document.getElementById(`type-${t}`).className = `flex-1 py-2 ${t===type ? 'bg-[--brand-coffee] text-[--brand-cream]' : ''}`;
  });
